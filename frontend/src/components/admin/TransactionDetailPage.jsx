  ...
  return (
    <section className="dashboard-main-content admin-main-content transaction-detail-page">
      ...
      <div className="transaction-detail-grid">
        <article className="panel-card transaction-summary-card"><h3>Payment summary</h3><div className="transaction-detail-fields"><DetailField label="Transaction ID">{transaction.transaction_number}</DetailField><DetailField label="Order ID">{order.order_number}</DetailField><DetailField label="Order date">{formatDate(order.created_at)}</DetailField><DetailField label="Amount">{formatMoney(transaction.amount)}</DetailField><DetailField label="Gateway">{transaction.payment_gateway}</DetailField><DetailField label="Payment method">{transaction.payment_method}</DetailField><DetailField label="Payment status"><span className={`status-badge ${transaction.status}`}>{transaction.status}</span></DetailField><DetailField label="Paid at">{formatDate(transaction.paid_at)}</DetailField></div></article>
        ...
        })}</div><div className="transaction-totals"><p><span>Subtotal</span><strong>{formatMoney(order.subtotal)}</strong></p><p><span>Shipping</span><strong>{formatMoney(order.shipping_amount)}</strong></p><p><span>Total</span><strong>{formatMoney(order.final_amount)}</strong></p></div></article>
      </div>
    </section>
  )
}
